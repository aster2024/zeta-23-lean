...
-/
import Zeta23.StrictImprovement.Q7SalvagedLocal
import Zeta23.StrictImprovement.ZetaFiniteCorrelation
import Zeta23.StrictImprovement.CoreTripleEnergy

/-!
# Core geometry and finite/full transfer for salvaged q7 superbin blocks
-/
...
def coreQ7SalvagedBlockCoordinate
    (hL : 0 < P.L T) (hC : 0 ≤ C)
    (q : Q7SalvagedBlock (coreQ6HalfBinEnumeration Z T C P hL hC))
    (r : Fin (q7SalvagedBlockSize q)) : ℝ :=
...
theorem coreQ7SalvagedBlock_geometry
    (hL : 0 < P.L T) (hC : 0 ≤ C)
    (q : Q7SalvagedBlock (coreQ6HalfBinEnumeration Z T C P hL hC)) :
    Q7SalvagedBlockGeometry q
      (coreQ7SalvagedBlockCoordinate Z T C P hL hC q) := by
  ...
  | fiveLeft =>
      ...
      have hi := coreQ6LeftEntry_bounds Z T C P hL hC b
        (wideRepairFiveSlot (q7SalvagedLeftOccupancy E b) u i)
      have hj := coreQ6LeftEntry_bounds Z T C P hL hC b
        (wideRepairFiveSlot (q7SalvagedLeftOccupancy E b) u j)
      exact abs_sub_le_sixteen_of_left_bounds hi hj
  | fiveRight =>
      intro i j
      have hi := coreQ6RightEntry_bounds Z T C P hL hC b
        (wideRepairFiveSlot (q7SalvagedRightOccupancy E b) u i)
      have hj := coreQ6RightEntry_bounds Z T C P hL hC b
        (wideRepairFiveSlot (q7SalvagedRightOccupancy E b) u j)
  ...
  | threeLeft =>
      ...
      have hj := coreQ6LeftEntry_bounds Z T C P hL hC b
        (wideRepairResidualSlot (q7SalvagedLeftOccupancy E b) 3 hcond.1 j)
      exact abs_sub_le_sixteen_of_left_bounds hi hj
  | threeRight =>
      ...
      have hj := coreQ6RightEntry_bounds Z T C P hL hC b
        (wideRepairResidualSlot (q7SalvagedRightOccupancy E b) 3 hcond.1 j)
      exact abs_sub_le_sixteen_of_right_bounds hi hj
  | fourLeft =>
      ...
      have hj := coreQ6LeftEntry_bounds Z T C P hL hC b
        (wideRepairResidualSlot (q7SalvagedLeftOccupancy E b) 4 hcond.1 j)
      exact abs_sub_le_sixteen_of_left_bounds hi hj
  | fourRight =>
      ...
      have hj := coreQ6RightEntry_bounds Z T C P hL hC b
        (wideRepairResidualSlot (q7SalvagedRightOccupancy E b) 4 hcond.1 j)
      exact abs_sub_le_sixteen_of_right_bounds hi hj
  | six =>
      have hcond : q7SalvagedLeftOccupancy E b % 5 = 3 ∧
          q7SalvagedRightOccupancy E b % 5 = 3 :=
      ...
      · intro i hi
        have hidx :
            q7SalvagedBlockIndex E ⟨⟨b, .six⟩, u⟩ i =
              E.entry (b, 0)
                (wideRepairResidualSlot (q7SalvagedLeftOccupancy E b) 3
                  hcond.1 ⟨i.val, hi⟩) := by
          dsimp only [q7SalvagedBlockIndex, q7SalvagedBlockCoordinate, id]
          rw [dif_pos hi]
        dsimp only [coreQ7SalvagedBlockCoordinate]
        rw [hidx]
      ...
      · intro i hi
        ...
        have hi6 : i.val < 6 := by
          simpa [q7SalvagedBlockSize, q7SalvagedBlockKindSize] using i.isLt
        have hiright : i.val - 3 < 3 := by omega
        have hidx :
            q7SalvagedBlockIndex E ⟨⟨b, .six⟩, u⟩ i =
              E.entry (b, 1)
                (wideRepairResidualSlot (q7SalvagedRightOccupancy E b) 3
  ...
  | sevenThreeFour =>
      ...
      · intro i hi
        have hidx :
            q7SalvagedBlockIndex E ⟨⟨b, .sevenThreeFour⟩, u⟩ i =
              E.entry (b, 0)
                (wideRepairResidualSlot (q7SalvagedLeftOccupancy E b) 3
                  hcond.1 ⟨i.val, hi⟩) := by
          dsimp only [q7SalvagedBlockIndex, q7SalvagedBlockCoordinate, id]
      ...
      · intro i hi
        ...
        have hidx :
            q7SalvagedBlockIndex E ⟨⟨b, .sevenThreeFour⟩, u⟩ i =
              E.entry (b, 1)
                (wideRepairResidualSlot (q7SalvagedRightOccupancy E b) 4
                  hcond.2 ⟨i.val - 3, hiright⟩) := by
          dsimp only [q7SalvagedBlockIndex, q7SalvagedBlockCoordinate, id]
          rw [dif_neg hnot]
        dsimp only [coreQ7SalvagedBlockCoordinate]
  ...
  | sevenFourThree =>
      ...
      · intro i hi
        have hidx :
            q7SalvagedBlockIndex E ⟨⟨b, .sevenFourThree⟩, u⟩ i =
              E.entry (b, 0)
                (wideRepairResidualSlot (q7SalvagedLeftOccupancy E b) 4
                  hcond.1 ⟨i.val, hi⟩) := by
          dsimp only [q7SalvagedBlockIndex, q7SalvagedBlockCoordinate, id]
          rw [dif_pos hi]
        dsimp only [coreQ7SalvagedBlockCoordinate]
...
theorem packedCoreQ7SalvagedBlock_traceNorm_lower
    ...
    (hmargin6 : scale * q7SalvagedSixReward +
        3 * Real.sqrt 6 *
          localCorrelationError T P cRho epsFull ≤ q7SalvagedSixReward)
    (hmargin7 : scale * q7SalvagedSevenReward +
        (7 * Real.sqrt 7 / 2) *
          localCorrelationError T P cRho epsFull ≤ q7SalvagedSevenReward)
    (q : Q7SalvagedBlock
  ...
  let idx := q7SalvagedBlockIndex E q
  let x := normalizedCoreVec Z T 3 P hconj
  let coord := coreQ7SalvagedBlockCoordinate Z T 3 P hL (by norm_num) q
  ...
  have hlocalNe : ∀ {r t : Fin (q7SalvagedBlockSize q)},
      r ≠ t → idx r ≠ idx t := by
    intro r t hrt heq
    have hp : (⟨q, r⟩ : Σ q, Fin (q7SalvagedBlockSize q)) = ⟨q, t⟩ :=
      (q7SalvagedBlockIndex_injective E) heq
    have hrt' : r = t := by
      cases hp
      rfl
    exact hrt hrt'
  ...
  have hcloseCoord : ∀ r t : Fin (q7SalvagedBlockSize q),
    ...
    have harg : coord r - coord t =
      ...
      dsimp [coord, idx, coreQ7SalvagedBlockCoordinate]
      ring
    rw [harg]
    exact h
  have hedgeAll : ∀ r t : Fin (q7SalvagedBlockSize q),
  ...
  apply q7SalvagedBlock_nearby_traceNorm_lower hcertificates q coord
    (coreQ7SalvagedBlock_geometry Z T 3 P hL (by norm_num) q)
