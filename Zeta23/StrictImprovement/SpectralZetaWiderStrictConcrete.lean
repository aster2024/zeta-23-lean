...
/-!
# Spectral-certificate strict improvement at fixed taper
-/
...
/-- At fixed admissible parameters, the spectral four-point certificate yields
the displayed simple-on-line density improvement. -/
theorem zeta_spectral_strict_simple_fixed_lam
  ...
  obtain ⟨theta0, hTail, htheta, hSeam, hgain⟩ :=
    zeta_spectral_strict_package_fixed_lam hcertificate
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => spectralAtDCoreGain zetaZeroConfig T P m)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [SpectralStrictSeamAt] using hSeam)
    hboundary
