...
-/
import Zeta23.StrictImprovement.Q6SuperbinZetaStrictSeamEventually
import Zeta23.ThmD.Mult

/-! # Q6 superbin zeta package at fixed taper -/
...
def Q6SuperbinStrictSeamAt (Z : ZeroConfig) (P : Params)
...
theorem zeta_q6Superbin_strict_package_fixed_lam
    ...
    ∃ theta0 : ℝ → ℝ,
      ...
      (∀ᶠ T in atTop,
        Q6SuperbinStrictSeamAt zetaZeroConfig P theta0 scale T) ∧
      (∀ᶠ T in atTop,
        (scale ^ 2 * wideRepairAlpha ^ 2 *
            (ThmD.HD lam - eps -
              q6SuperbinPackingLoss * (lam + eps) - eps) ^ 2 /
            (1 + eps)) * (Ncount T (2 * T) : ℝ) ≤
          q6SuperbinAtDCoreGain zetaZeroConfig T P scale) := by
  dsimp
  ...
  have hseam := eventually_seamA_mult2_atD_with_q6Superbin_gain
  ...
  have hgain := eventually_q6SuperbinAtDCoreGain_ge_fixed_eps
  ...
  · simpa [Q6SuperbinStrictSeamAt] using hseam
