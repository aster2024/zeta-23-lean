...
/-!
# Direct-certificate strict improvement at fixed taper
-/
...
/-- At fixed admissible parameters, the direct four-point certificate yields
the displayed zeta simple-on-line density improvement. -/
theorem zeta_direct_strict_simple_fixed_lam
  ...
  obtain ⟨theta₀, hTail, htheta, hSeam, hgain⟩ :=
    zeta_direct_strict_package_fixed_lam hcertificate
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => directAtDCoreGain zetaZeroConfig T P)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [DirectStrictSeamAt] using hSeam)
    hboundary
