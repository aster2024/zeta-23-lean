...
/-!
# Abstract dependent-block pinching for the full-spectral q7 superbin repair
...
theorem q7FullSpectral_selected_blocks_traceNorm_lower
...
theorem q7FullSpectral_selected_blocks_traceNorm_target_lower
    ...
    2 * (scale * wideRepairAlpha * max 0
      ...
      Tail.traceNorm (gramDeviation_isHermitian x) := by
  have hrewards := sum_q7FullSpectralPairReward_target_lower
    left right hcover D hbins
  rw [← hrewardSum] at hrewards
  have hscaled := mul_le_mul_of_nonneg_left hrewards hscale
  have htrace := q7FullSpectral_selected_blocks_traceNorm_lower
...
theorem rank_trace_two_with_q7FullSpectral_selected_blocks
  ...
  let target : ℝ := max 0 ((Fintype.card S : ℝ) -
    q7FullSpectralPackingLoss * D - q7FullSpectralPackingIntercept)
  ...
  have htrace : 2 * R ≤ Tail.traceNorm (gramDeviation_isHermitian x) := by
    dsimp [R, target]
    exact q7FullSpectral_selected_blocks_traceNorm_target_lower
...
theorem rank_trace_two_with_normalized_q7FullSpectral_selected_blocks
    {S B C d : Type*} [Fintype S] [DecidableEq S] [Nonempty S]
    [Fintype B] [DecidableEq B]
    [Fintype C] [DecidableEq C] [Fintype d] [DecidableEq d]
    (left right : B → ℕ)
  ...
  have hmain := rank_trace_two_with_q7FullSpectral_selected_blocks
