...
-/
import Zeta23.StrictImprovement.Q7FullSpectralCoreBlocks
import Zeta23.StrictImprovement.Q7FullSpectralPacking
import Zeta23.StrictImprovement.ZetaBoundaryLoss
import Zeta23.ZeroSide.Mult
import Zeta23.ThmD.ParamsD

/-!
# Fixed-height zero-side defect with q6 superbin repair
-/
...
def q7FullSpectralAtDCoreGain
  ...
  scale ^ 2 * wideRepairAlpha ^ 2 /
    ...
    max 0 ((Fintype.card (CoreSimpleLabel Z T 3) : ℝ) -
      q7FullSpectralPackingLoss * coreBinD T (P.atD T) -
      q7FullSpectralPackingIntercept) ^ 2

/-- Fixed-height `c=2` zero-side inequality with the q6 superbin gain. -/
theorem hatAz_mult2_with_q7FullSpectral_gain
    ...
    (hlocal : ∀ q : Q7FullSpectralBlock
      (coreQ6HalfBinEnumeration Z T 3 P hL (by norm_num)),
      2 * (scale * q7FullSpectralBlockReward q) ≤ Tail.traceNorm
    ...
    4 * rtrace (P.hat T (Z.Az P T)) -
        ...
        scale ^ 2 * wideRepairAlpha ^ 2 /
          (Fintype.card (CoreSimpleLabel Z T 3) : ℝ) *
          max 0 ((Fintype.card (CoreSimpleLabel Z T 3) : ℝ) -
            q7FullSpectralPackingLoss * coreBinD T P -
            q7FullSpectralPackingIntercept) ^ 2
  ...
  have hcover : ∑ b, (q7FullSpectralLeftOccupancy E b +
      q7FullSpectralRightOccupancy E b) =
      Fintype.card (CoreSimpleLabel Z T 3) := by
    dsimp [E]
    exact coreQ6HalfBinEnumeration_superbin_cover
      Z T 3 P hL (by norm_num)
  have hbins := coreQ7FullSpectral_card_le T P hL hT
  ...
  have hmain := rank_trace_two_with_normalized_q7FullSpectral_selected_blocks
    (q7FullSpectralLeftOccupancy E) (q7FullSpectralRightOccupancy E)
    hcover (coreBinD T P) hbins q7FullSpectralBlockSize
    (q7FullSpectralBlockIndex E) (q7FullSpectralBlockIndex_injective E)
    q7FullSpectralBlockReward (sum_q7FullSpectralBlockReward_eq E)
    x hunit hscale hlocal wgt hw0 hw1 hA hb
