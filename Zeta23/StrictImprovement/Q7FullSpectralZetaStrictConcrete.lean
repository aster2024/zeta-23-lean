...
import Zeta23.StrictImprovement.Q7FullSpectralZetaStrictFixedLambda
...
theorem zeta_q7FullSpectral_strict_simple_fixed_lam
  ...
  obtain ⟨theta0, hTail, htheta, hSeam, hgain⟩ :=
    zeta_q7FullSpectral_strict_package_fixed_lam hcertificates
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => q7FullSpectralAtDCoreGain zetaZeroConfig T P scale)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [Q7FullSpectralStrictSeamAt] using hSeam)
    hboundary
