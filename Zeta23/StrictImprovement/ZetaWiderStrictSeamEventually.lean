...
import Zeta23.StrictImprovement.ZetaWiderCoreGain

/-!
# Eventual discharge of the width-six four-point seam conditions

For a fixed valid Montgomery--Taylor family, this file packages the same
large-height analytic conditions as the three-point route and inserts the
wider local defect.  No stronger tail or Poisson hypothesis is introduced.
...
/-- The wider strict seam inequality holds eventually once the selected core
is nonempty. -/
theorem eventually_seamA_mult2_atD_with_wider_explicit_gain
    ...
    (hdelta : 18 * (1 - P.lam) < widerDeltaLower) :
    ∀ᶠ T in atTop,
  ...
  have hdeltaEv := eventually_widerAtDLocalDelta_pos hP hdelta
  ...
  have hseam := seamA_mult2_atD_with_wider_explicit_gain
    Z hP hconj hreal hPois hF hTl hT hc hbudget h16 hpos hdeltaT.le
  simpa [widerAtDCoreGain] using hseam
