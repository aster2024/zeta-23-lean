...
-/
import Zeta23.StrictImprovement.Q7FullSpectralZetaLocalDefect
...
theorem q7FullSpectral_quadratic_core_gain_lower
    {s N D scale h d r u q : ℝ}
    (hs : 0 < s) (hN : 0 < N) (hu : 0 < u)
    (hscale : 0 ≤ scale)
    (hslo : h * N ≤ s) (hshi : s ≤ u * N)
    (hD : D ≤ d * N)
    (hconst : q7FullSpectralPackingIntercept ≤ r * N)
    (hq : q = h - q7FullSpectralPackingLoss * d - r) (hq0 : 0 ≤ q) :
    scale ^ 2 * wideRepairAlpha ^ 2 / s *
        max 0 (s - q7FullSpectralPackingLoss * D -
          q7FullSpectralPackingIntercept) ^ 2 ≥
      scale ^ 2 * wideRepairAlpha ^ 2 * q ^ 2 / u * N := by
  have hqN0 : 0 ≤ q * N := mul_nonneg hq0 hN.le
  have ha : q * N ≤ s - q7FullSpectralPackingLoss * D -
      q7FullSpectralPackingIntercept := by
    rw [hq]
    have hloss := q7FullSpectralPackingLoss_nonneg
    nlinarith
  have ha0 : 0 ≤ s - q7FullSpectralPackingLoss * D -
      q7FullSpectralPackingIntercept := hqN0.trans ha
  rw [max_eq_right ha0]
  have hsq : (q * N) ^ 2 ≤
      (s - q7FullSpectralPackingLoss * D -
        q7FullSpectralPackingIntercept) ^ 2 :=
  ...
  rw [show scale ^ 2 * wideRepairAlpha ^ 2 / s *
      ...
      scale ^ 2 * wideRepairAlpha ^ 2 *
        (s - q7FullSpectralPackingLoss * D -
          q7FullSpectralPackingIntercept) ^ 2 / s by ring]
  ...
  calc
    ...
    _ ≤ scale ^ 2 * wideRepairAlpha ^ 2 *
        (s - q7FullSpectralPackingLoss * D -
          q7FullSpectralPackingIntercept) ^ 2 := hnum

theorem eventually_q7FullSpectralAtDCoreGain_ge_of_bounds
    ...
    (hconst : ∀ᶠ T in atTop,
      q7FullSpectralPackingIntercept ≤ r * (Z.N T (2 * T) : ℝ)) :
    ∀ᶠ T in atTop,
      (scale ^ 2 * wideRepairAlpha ^ 2 * q ^ 2 / u) *
          (Z.N T (2 * T) : ℝ) ≤
        q7FullSpectralAtDCoreGain Z T P scale := by
  ...
  have hgain := q7FullSpectral_quadratic_core_gain_lower
    hs hNT hu hscale hsloT hshiT hDT hconstT hq hq0
  simpa [q7FullSpectralAtDCoreGain, s] using hgain

theorem eventually_q7FullSpectralAtDCoreGain_ge_fixed_eps
    ...
    (hH : 0 < H - eps)
    (hscale : 0 ≤ scale)
    (hq0 : 0 ≤ H - eps - q7FullSpectralPackingLoss *
      (P.lam + eps) - eps) :
    ∀ᶠ T in atTop,
      (scale ^ 2 * wideRepairAlpha ^ 2 *
          (H - eps - q7FullSpectralPackingLoss * (P.lam + eps) - eps) ^ 2 /
          (1 + eps)) * (Z.N T (2 * T) : ℝ) ≤
        q7FullSpectralAtDCoreGain Z T P scale := by
  ...
  have hconst : ∀ᶠ T in atTop,
      q7FullSpectralPackingIntercept ≤ eps * (Z.N T (2 * T) : ℝ) := by
    filter_upwards [hNtop.eventually_ge_atTop
      (q7FullSpectralPackingIntercept / eps)] with T h
    have h' : q7FullSpectralPackingIntercept ≤
        (Z.N T (2 * T) : ℝ) * eps :=
      (div_le_iff₀ heps).mp h
    nlinarith
  exact eventually_q7FullSpectralAtDCoreGain_ge_of_bounds Z P
