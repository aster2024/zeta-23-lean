...
-/
import Zeta23.StrictImprovement.Q7FullSpectralZetaSeamAtD
import Zeta23.StrictImprovement.Q7FullSpectralZetaCoreGain
import Zeta23.StrictImprovement.ZetaLocalDeltaLimit

/-! # Eventual full-spectral q7 superbin seam at fixed taper -/
...
theorem eventually_seamA_mult2_atD_with_q7FullSpectral_gain
    ...
    (hmargin6 : scale * q7FullSpectralSixReward +
        9 * Real.sqrt 6 * (1 - P.lam) < q7FullSpectralSixReward)
    (hmargin7 : scale * q7FullSpectralSevenReward +
        (21 * Real.sqrt 7 / 2) * (1 - P.lam) < q7FullSpectralSevenReward) :
    ∀ᶠ T in atTop,
  ...
  have hmargin6Tendsto : Tendsto
      (fun T => scale * q7FullSpectralSixReward + 3 * Real.sqrt 6 *
          localCorrelationError T (P.atD T) (ThmD.cDT P.ϱ P.lam)
            (12 * P.w / P.L T + 3 * (1 - P.lam))) atTop
      (nhds (scale * q7FullSpectralSixReward +
        9 * Real.sqrt 6 * (1 - P.lam))) := by
    have hconst : Tendsto
        (fun _ : ℝ => scale * q7FullSpectralSixReward) atTop
        (nhds (scale * q7FullSpectralSixReward)) := tendsto_const_nhds
    have hsum := hconst.add (herr.const_mul (3 * Real.sqrt 6))
    have hend : scale * q7FullSpectralSixReward +
        (3 * Real.sqrt 6) * (3 * (1 - P.lam)) =
          scale * q7FullSpectralSixReward +
            9 * Real.sqrt 6 * (1 - P.lam) := by ring
    rw [← hend]
    simpa only using hsum
  have hmargin7Tendsto : Tendsto
      (fun T => scale * q7FullSpectralSevenReward + (7 * Real.sqrt 7 / 2) *
          localCorrelationError T (P.atD T) (ThmD.cDT P.ϱ P.lam)
            (12 * P.w / P.L T + 3 * (1 - P.lam))) atTop
      (nhds (scale * q7FullSpectralSevenReward +
        (21 * Real.sqrt 7 / 2) * (1 - P.lam))) := by
    have hconst : Tendsto
        (fun _ : ℝ => scale * q7FullSpectralSevenReward) atTop
        (nhds (scale * q7FullSpectralSevenReward)) := tendsto_const_nhds
    have hsum := hconst.add (herr.const_mul (7 * Real.sqrt 7 / 2))
    have hend : scale * q7FullSpectralSevenReward +
        (7 * Real.sqrt 7 / 2) * (3 * (1 - P.lam)) =
          scale * q7FullSpectralSevenReward +
            (21 * Real.sqrt 7 / 2) * (1 - P.lam) := by ring
    rw [← hend]
    simpa only using hsum
  ...
  have hmargin7Ev := hmargin7Tendsto.eventually (Iio_mem_nhds hmargin7)
  filter_upwards [hTail, hNonempty, eventually_ge_atTop T0,
      ...
      haRange, hmargin3Ev, hmargin4Ev, hmargin5Ev, hmargin6Ev, hmargin7Ev,
      eventually_gt_atTop (0 : ℝ)]
    with T hTl hne hT0 h16 hlarge haD hm3 hm4 hm5 hm6 hm7 hT
  ...
  have hseam := seamA_mult2_atD_with_q7FullSpectral_gain
    hcertificates Z hP hconj hreal hPois hF hTl hT hc hbudget h16 hpos
      hscale hm3.le hm4.le hm5.le hm6.le hm7.le
  simpa [q7FullSpectralAtDCoreGain] using hseam
