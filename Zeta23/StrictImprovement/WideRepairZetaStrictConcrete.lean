...
import Zeta23.StrictImprovement.WideRepairZetaStrictFixedLambda

/-! # Mixed repaired strict improvement at fixed taper -/
...
theorem zeta_wide_repair_strict_simple_fixed_lam
  ...
  obtain ⟨theta0, hTail, htheta, hSeam, hgain⟩ :=
    zeta_wide_repair_strict_package_fixed_lam hcertificates
      hlam0 hlam1 heps hH hscale hmargin3 hmargin4 hmargin5 hq0
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => wideRepairAtDCoreGain zetaZeroConfig T P scale)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [WideRepairStrictSeamAt] using hSeam)
    hboundary
