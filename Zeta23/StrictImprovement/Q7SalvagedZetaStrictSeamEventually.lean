...
-/
import Zeta23.StrictImprovement.Q7SalvagedZetaSeamAtD
import Zeta23.StrictImprovement.Q7SalvagedZetaCoreGain
import Zeta23.StrictImprovement.ZetaLocalDeltaLimit

/-! # Eventual salvaged q7 superbin seam at fixed taper -/
...
theorem eventually_seamA_mult2_atD_with_q7Salvaged_gain
    ...
    (hmargin6 : scale * q7SalvagedSixReward +
        9 * Real.sqrt 6 * (1 - P.lam) < q7SalvagedSixReward)
    (hmargin7 : scale * q7SalvagedSevenReward +
        (21 * Real.sqrt 7 / 2) * (1 - P.lam) < q7SalvagedSevenReward) :
    ∀ᶠ T in atTop,
      4 * rtrace ((P.atD T).hat T (Z.Gz (P.atD T) T)) -
          ...
          theta0 T / ((P.atD T).a T * (P.atD T).L T) *
            (4 + 2 * Real.sqrt
                (frobSq ((P.atD T).hat T (Z.Gz (P.atD T) T))) +
              theta0 T / ((P.atD T).a T * (P.atD T).L T)) +
          q7SalvagedAtDCoreGain Z T P scale
  ...
  have hmargin6Tendsto : Tendsto
      (fun T => scale * q7SalvagedSixReward + 3 * Real.sqrt 6 *
          localCorrelationError T (P.atD T) (ThmD.cDT P.ϱ P.lam)
            (12 * P.w / P.L T + 3 * (1 - P.lam))) atTop
      (nhds (scale * q7SalvagedSixReward +
        9 * Real.sqrt 6 * (1 - P.lam))) := by
    have hconst : Tendsto
        (fun _ : ℝ => scale * q7SalvagedSixReward) atTop
        (nhds (scale * q7SalvagedSixReward)) := tendsto_const_nhds
    have hsum := hconst.add (herr.const_mul (3 * Real.sqrt 6))
    have hend : scale * q7SalvagedSixReward +
        (3 * Real.sqrt 6) * (3 * (1 - P.lam)) =
          scale * q7SalvagedSixReward +
            9 * Real.sqrt 6 * (1 - P.lam) := by ring
    rw [← hend]
    simpa only using hsum
  have hmargin7Tendsto : Tendsto
      (fun T => scale * q7SalvagedSevenReward + (7 * Real.sqrt 7 / 2) *
          localCorrelationError T (P.atD T) (ThmD.cDT P.ϱ P.lam)
            (12 * P.w / P.L T + 3 * (1 - P.lam))) atTop
      (nhds (scale * q7SalvagedSevenReward +
        (21 * Real.sqrt 7 / 2) * (1 - P.lam))) := by
    have hconst : Tendsto
        (fun _ : ℝ => scale * q7SalvagedSevenReward) atTop
        (nhds (scale * q7SalvagedSevenReward)) := tendsto_const_nhds
    have hsum := hconst.add (herr.const_mul (7 * Real.sqrt 7 / 2))
    have hend : scale * q7SalvagedSevenReward +
        (7 * Real.sqrt 7 / 2) * (3 * (1 - P.lam)) =
          scale * q7SalvagedSevenReward +
