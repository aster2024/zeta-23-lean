...
import Zeta23.StrictImprovement.ZetaWiderStrictFixedLambda

/-!
# Unconditional width-six strict improvement at fixed lambda

The frozen trace/Frobenius endgame is independent of the shape of the positive
gain.  This file instantiates it with the exact width-six four-point gain.
...
simple-on-line density improves by the width-six quadratic amount. -/
theorem zeta_wider_strict_simple_fixed_lam
    {lam delta₀ eps : ℝ}
  ...
  obtain ⟨theta₀, hTail, htheta, hSeam, hgain⟩ :=
    zeta_wider_strict_package_fixed_lam
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => widerAtDCoreGain zetaZeroConfig T P)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [WiderStrictSeamAt] using hSeam)
    hboundary
