...
-/
import Zeta23.StrictImprovement.Q7SalvagedZetaStrictSeamEventually
...
def Q7SalvagedStrictSeamAt (Z : ZeroConfig) (P : Params)
...
theorem zeta_q7Salvaged_strict_package_fixed_lam
    ...
    (hmargin6 : scale * q7SalvagedSixReward +
        9 * Real.sqrt 6 * (1 - lam) < q7SalvagedSixReward)
    (hmargin7 : scale * q7SalvagedSevenReward +
        (21 * Real.sqrt 7 / 2) * (1 - lam) < q7SalvagedSevenReward)
    (hq0 : 0 ≤ ThmD.HD lam - eps -
      q7SalvagedPackingLoss * (lam + eps) - eps) :
    ...
    ∃ theta0 : ℝ → ℝ,
      ...
      (∀ᶠ T in atTop,
        Q7SalvagedStrictSeamAt zetaZeroConfig P theta0 scale T) ∧
      (∀ᶠ T in atTop,
        (scale ^ 2 * wideRepairAlpha ^ 2 *
            (ThmD.HD lam - eps -
              q7SalvagedPackingLoss * (lam + eps) - eps) ^ 2 /
            (1 + eps)) * (Ncount T (2 * T) : ℝ) ≤
          q7SalvagedAtDCoreGain zetaZeroConfig T P scale) := by
  dsimp
  ...
  have hseam := eventually_seamA_mult2_atD_with_q7Salvaged_gain
  ...
  have hgain := eventually_q7SalvagedAtDCoreGain_ge_fixed_eps
  ...
  · simpa [Q7SalvagedStrictSeamAt] using hseam
