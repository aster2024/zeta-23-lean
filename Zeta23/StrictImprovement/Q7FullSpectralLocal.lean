...
/-!
# Local trace-norm inequality for every full-spectral q7 inventory block
...
def Q7FullSpectralBlockGeometry
    {S B : Type*} [Fintype B]
    {E : BinnedEnumeration S (B × Fin 2)}
    (q : Q7FullSpectralBlock E)
    (coord : Fin (q7FullSpectralBlockSize q) → ℝ) : Prop :=
  match q.1.2 with
  | .six =>
...
theorem q7FullSpectralBlock_nearby_traceNorm_lower
    {S K : Type*} [Fintype K]
    {E : BinnedEnumeration S (K × Fin 2)}
    (hcertificates : Q7FullSpectralEndpointCertificates)
    (q : Q7FullSpectralBlock E)
    (coord : Fin (q7FullSpectralBlockSize q) → ℝ)
    (hgeometry : Q7FullSpectralBlockGeometry q coord)
    {B : Matrix (Fin (q7FullSpectralBlockSize q))
      (Fin (q7FullSpectralBlockSize q)) ℂ}
    ...
    2 * (scale * q7FullSpectralBlockReward q) ≤ Tail.traceNorm hB := by
  rcases q with ⟨k, u⟩
  ...
  | fourRight =>
      ...
      exact wideRepairFour_nearby_traceNorm_lower
        hcertificates.base.four coord hdist hB hdiagB heps hmargin4
          ...
          (hedge (by decide)) (hedge (by decide)) (hedge (by decide))
  | six =>
      exact q7FullSpectral_six_nearby_traceNorm_lower hcertificates.six coord
        hgeometry.1 hgeometry.2 hB heps hclose hmargin6
  | sevenThreeFour =>
      exact q7FullSpectral_seven_threeFour_nearby_traceNorm_lower
        hcertificates.seven coord hgeometry.1 hgeometry.2 hB heps hclose hmargin7
  | sevenFourThree =>
      exact q7FullSpectral_seven_fourThree_nearby_traceNorm_lower
        hcertificates.seven coord hgeometry.1 hgeometry.2 hB heps hclose hmargin7
