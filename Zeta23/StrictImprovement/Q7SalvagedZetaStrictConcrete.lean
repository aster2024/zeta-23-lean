...
/-! # Salvaged q7 superbin strict improvement at fixed taper -/
...
theorem zeta_q7Salvaged_strict_simple_fixed_lam
  ...
  obtain ⟨theta0, hTail, htheta, hSeam, hgain⟩ :=
    zeta_q7Salvaged_strict_package_fixed_lam hcertificates
      hlam0 hlam1 heps hH hscale hmargin3 hmargin4 hmargin5 hmargin6 hmargin7 hq0
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => q7SalvagedAtDCoreGain zetaZeroConfig T P scale)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [Q7SalvagedStrictSeamAt] using hSeam)
    hboundary
