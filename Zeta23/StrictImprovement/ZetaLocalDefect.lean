...
/-!
# Fixed-height strengthened multiplicity inequality

This module inserts the canonical core triples into the `c=2`
multiplicity-aware zero-side inequality.  The exact boundary bookkeeping is
honest: removing the width-three ends costs one additional
`excludedSimpleCount` on the right-hand side.
...
`D=L*T/(2*pi)`.  This is a source draft pending the pinned Lean build.
-/
...
/-- The fixed-height `c=2` zero-side inequality with the exact local Gram
gain and the explicit boundary-simple correction. -/
theorem hatAz_mult2_with_core_gain
  ...
  have hcover : ∑ b, E.occupancy b = Fintype.card (CoreSimpleLabel Z T 3) := by
    dsimp [E]
    exact coreBinnedEnumeration_cover Z T 3 P hL (by norm_num)
  have hbins :
      (Fintype.card (Fin (intervalBinCount (coreBinD T P))) : ℝ) ≤
        coreBinD T P / 4 + 1 :=
    coreIntervalBin_card_le T P hL hT
  ...
  have hmain := rank_trace_two_with_normalized_binned_family
