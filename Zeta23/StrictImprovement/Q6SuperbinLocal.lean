...
/-!
# Local trace-norm inequality for every q6 inventory block

The three-, four-, and five-block cases require only a width-`16*pi`
diameter bound.  The six-block case records the left/right half geometry of a
single width-`32*pi` superbin.
-/
...
def Q6SuperbinBlockGeometry
    {S B : Type*} [Fintype B]
    {E : BinnedEnumeration S (B × Fin 2)}
    (q : Q6SuperbinBlock E)
    (coord : Fin (q6SuperbinBlockSize q) → ℝ) : Prop :=
  match q.1.2 with
  | .six =>
...
theorem q6SuperbinBlock_nearby_traceNorm_lower
    {S K : Type*} [Fintype K]
    {E : BinnedEnumeration S (K × Fin 2)}
    (hcertificates : Q6SuperbinEndpointCertificates)
    (q : Q6SuperbinBlock E)
    (coord : Fin (q6SuperbinBlockSize q) → ℝ)
    (hgeometry : Q6SuperbinBlockGeometry q coord)
    {B : Matrix (Fin (q6SuperbinBlockSize q))
      (Fin (q6SuperbinBlockSize q)) ℂ}
    ...
    2 * (scale * q6SuperbinBlockReward q) ≤ Tail.traceNorm hB := by
  rcases q with ⟨k, u⟩
  ...
  | fourRight =>
      ...
      exact wideRepairFour_nearby_traceNorm_lower
        hcertificates.base.four coord hdist hB hdiagB heps hmargin4
          ...
          (hedge (by decide)) (hedge (by decide)) (hedge (by decide))
  | six =>
      exact q6Superbin_nearby_traceNorm_lower hcertificates.six coord
