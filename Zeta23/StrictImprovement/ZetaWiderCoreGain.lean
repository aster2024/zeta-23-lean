...
/-!
# Linear density supplied by the width-six four-point gain

This file inserts the wider local defect into the same exact core-density and
Riemann--von Mangoldt bounds used by the three-point route.  The coefficient is
`1/8` and the additive packing remainder is `3`.
...
/-- The exact quadratic gain in the width-six four-point seam theorem. -/
def widerAtDCoreGain (Z : ZeroConfig) (T : ℝ) (P : Params) : ℝ :=
...
/-- Eventual abstract lower bound for the wider quadratic gain. -/
theorem eventually_widerAtDCoreGain_ge_of_bounds
    (Z : ZeroConfig) (P : Params)
    {delta₀ h d r u q : ℝ}
    (hdelta₀ : 0 ≤ delta₀) (hh : 0 < h) (hu : 0 < u)
    (hq : q = h - d / 2 - r) (hq0 : 0 ≤ q)
    (hN : ∀ᶠ T in atTop, 0 < (Z.N T (2 * T) : ℝ))
    (hdelta : ∀ᶠ T in atTop, delta₀ ≤ widerAtDLocalDelta T P)
    (hslo : ∀ᶠ T in atTop,
      h * (Z.N T (2 * T) : ℝ) ≤
  ...
  simpa [widerAtDCoreGain, s] using hgain

/-- Concrete fixed-epsilon gain.  The margin records the selected-core loss,
the bin-density loss, and the additive packing remainder separately. -/
theorem eventually_widerAtDCoreGain_ge_fixed_eps
    ...
    (hH : 0 < H - eps)
    (hdelta₀ : 0 ≤ delta₀)
    (hdeltaLim : delta₀ < widerDeltaLower - 18 * (1 - P.lam))
  ...
  have hdelta : ∀ᶠ T in atTop, delta₀ ≤ widerAtDLocalDelta T P :=
    (eventually_widerAtDLocalDelta_gt hP hdeltaLim).mono fun T h => h.le
  ...
  have hconst : ∀ᶠ T in atTop,
    ...
    nlinarith
  exact eventually_widerAtDCoreGain_ge_of_bounds Z P
