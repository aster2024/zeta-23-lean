...
/-!
# Eventual direct four-point seam
-/
...
/-- The strict seam supplied by the direct endpoint certificate holds
eventually for each fixed valid taper. -/
theorem eventually_seamA_mult2_atD_with_direct_explicit_gain
    ...
    (hdelta : 18 * (1 - P.lam) < directFourEnergyLower / 2) :
    ∀ᶠ T in atTop,
  ...
  have hdeltaEv := eventually_directAtDLocalDelta_pos hP hdelta
  ...
  have hseam := seamA_mult2_atD_with_direct_explicit_gain
    hcertificate Z hP hconj hreal hPois hF hTl hT hc hbudget h16 hpos
      hdeltaT.le
  simpa [directAtDCoreGain] using hseam
