...
/-!
# Concrete fixed-height seam for the full certificate diameter
-/
...
/-- Concrete `atD` seam for the full `264/7` certificate domain. -/
theorem seamA_mult2_atD_with_certificate_slack_spectral_gain
  ...
  have haD : 0 < (P.atD T).a T := by
    ...
    nlinarith
  apply seamA_mult2_with_certificate_slack_spectral_gain Z T (P.atD T)
    hconj hreal hPois hTl haD hLD hT.le hpos hm
  intro q
  exact packedCoreCertificateSlackFour_atD_spectral_local_traceNorm
