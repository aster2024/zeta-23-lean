...
/-!
# Eventual spectral four-point seam

The taper is fixed before height tends to infinity.  The finite/full error
tends to `3*(1-lam)`, so the local trace-norm margin costs exactly
`6*sqrt 3*(1-lam)`.
-/
...
/-- The spectral seam holds eventually for every fixed nonnegative mass whose
limiting square-root margin is strict. -/
theorem eventually_seamA_mult2_atD_with_spectral_explicit_gain
  ...
  have hseam := seamA_mult2_atD_with_spectral_explicit_gain
    hcertificate Z hP hconj hreal hPois hF hTl hT hc hbudget h16 hpos
      hm hmarginT.le
  simpa [spectralAtDCoreGain] using hseam
