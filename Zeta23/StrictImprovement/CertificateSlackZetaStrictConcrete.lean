...
/-!
# Full-certificate strict improvement at fixed taper
-/
...
/-- At fixed admissible parameters, the full certificate diameter yields the
displayed simple-on-line density improvement. -/
theorem zeta_certificate_slack_spectral_strict_simple_fixed_lam
  ...
  obtain ⟨theta0, hTail, htheta, hSeam, hgain⟩ :=
    zeta_certificate_slack_spectral_strict_package_fixed_lam hcertificate
      hlam0 hlam1 heps hH hm hmarginLim hq0
  ...
  have hstrict := strict_thmD_mult2_abstract
    ...
    (fun T => certificateSlackSpectralAtDCoreGain
      zetaZeroConfig T P m)
    (fun T => (excludedSimpleCount zetaZeroConfig T 3 : ℝ))
    (by simpa [CertificateSlackSpectralStrictSeamAt] using hSeam)
    hboundary
