...
/-!
# Spectral endpoint certificate on every widened-bin packed core four-tuple

This is the finite/full correlation seam for the full rational simplex retained
by the interval certificate.  The certificate itself remains an explicit
argument.
-/
...
/-- A packed finite core block in one `264/7` bin inherits spectral
trace-norm mass `m` once the square-root margin absorbs the full finite/full
correlation error. -/
theorem packedCoreCertificateSlackFour_spectral_traceNorm_lower
    ...
    (q : PackedFour (coreCertificateSlackEnumeration Z T 3 P hL (by norm_num))) :
    2 * Real.sqrt m ≤
      Tail.traceNorm
        ((gramDeviation_isHermitian
          (normalizedCoreVec Z T 3 P hconj)).submatrix
            (packedFourIndex
              (coreCertificateSlackEnumeration Z T 3 P hL (by norm_num)) q)) := by
  ...
  have hdist : ∀ r t : Fin 4,
      |coord r - coord t| ≤ (264 : ℝ) / 7 := by
    intro r t
    exact (coreCertificateSlackPackedFour_pair_distance_lt
  ...
  have hedge : ∀ {r t : Fin 4}, r ≠ t →
    ...
    exact hclose r t
  apply spectral_slack_endpoint_to_nearby_traceNorm_lower
