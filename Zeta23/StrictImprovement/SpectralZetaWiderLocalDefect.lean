...
/-!
# Fixed-height multiplicity inequality with spectral four-point gain

This is the trace-norm counterpart of `ZetaWiderLocalDefect.lean`.  A local
half-trace-norm mass `sqrt m` on every packed width-six four-tuple contributes

`m/(16*s) * max 0 (s-D/2-3)^2`

to the finite-height rank--trace inequality.
-/
...
/-- The fixed-height `c=2` zero-side inequality with the spectral four-point
mass and the width-three boundary-simple correction. -/
theorem hatAz_mult2_with_spectral_core_gain
  ...
  have hmain := rank_trace_two_with_normalized_binned_spectral_fours
    E hcover (coreBinD T P) hbins x hunit hm hlocal
    wgt hw0 hw1 hA hb
