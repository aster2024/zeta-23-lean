...
/-!
# Abstract dependent-block pinching for the q6 superbin repair
...
theorem q6_selected_blocks_traceNorm_lower
...
theorem q6_selected_blocks_traceNorm_target_lower
...
/-- Rank--trace consequence of any concrete q6 superbin selector. -/
theorem rank_trace_two_with_q6_selected_blocks
  ...
  let target : ℝ := max 0 ((Fintype.card S : ℝ) -
    q6SuperbinPackingLoss * D - q6SuperbinPackingIntercept)
  ...
  have htrace : 2 * R ≤ Tail.traceNorm (gramDeviation_isHermitian x) := by
    dsimp [R, target]
    exact q6_selected_blocks_traceNorm_target_lower
...
concrete q6 selector supplies its local block inequalities. -/
theorem rank_trace_two_with_normalized_q6_selected_blocks
    {S B C d : Type*} [Fintype S] [DecidableEq S] [Nonempty S]
    [Fintype B] [DecidableEq B]
    [Fintype C] [DecidableEq C] [Fintype d] [DecidableEq d]
    (left right : B → ℕ)
  ...
  have hmain := rank_trace_two_with_q6_selected_blocks
