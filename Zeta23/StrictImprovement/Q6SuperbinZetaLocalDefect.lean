...
-/
import Zeta23.StrictImprovement.Q6SuperbinCoreBlocks
...
theorem hatAz_mult2_with_q6Superbin_gain
    ...
    (hlocal : ∀ q : Q6SuperbinBlock
      (coreQ6HalfBinEnumeration Z T 3 P hL (by norm_num)),
      2 * (scale * q6SuperbinBlockReward q) ≤ Tail.traceNorm
    ...
    4 * rtrace (P.hat T (Z.Az P T)) -
        ...
        scale ^ 2 * wideRepairAlpha ^ 2 /
          (Fintype.card (CoreSimpleLabel Z T 3) : ℝ) *
          max 0 ((Fintype.card (CoreSimpleLabel Z T 3) : ℝ) -
            q6SuperbinPackingLoss * coreBinD T P -
            q6SuperbinPackingIntercept) ^ 2
  ...
  have hcover : ∑ b, (q6SuperbinLeftOccupancy E b +
      q6SuperbinRightOccupancy E b) =
      Fintype.card (CoreSimpleLabel Z T 3) := by
    dsimp [E]
    exact coreQ6HalfBinEnumeration_superbin_cover
      Z T 3 P hL (by norm_num)
  have hbins := coreQ6Superbin_card_le T P hL hT
  ...
  have hmain := rank_trace_two_with_normalized_q6_selected_blocks
    ...
    hcover (coreBinD T P) hbins q6SuperbinBlockSize
    (q6SuperbinBlockIndex E) (q6SuperbinBlockIndex_injective E)
    q6SuperbinBlockReward (sum_q6SuperbinBlockReward_eq E)
